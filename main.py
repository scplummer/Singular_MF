...
def train(args, P, writer=None):
    ...
    for epoch in range(1, args.epochs):
        ...
        if epoch % args.display_interval == 0:
            ...
            print('epoch {}: elbo {}, nSn {}, gap {}'
                  .format(epoch, elbo, args.nSn, elbo + args.nSn))
...
def main():
    ...
    parser.add_argument('--data', nargs='*', default=['tanh_zeromean', 1], 
                        help='[0]: tanh or rr '
                             '[1]: H ') 
    parser.add_argument('--ns', nargs='*', default=[1000,2000,3000,4000,5000,6000,7000,8000,9000,10000])
    parser.add_argument('--prior_dist', nargs='*', default=[0, 1], help='only supports Gaussian, pass in mean and variance')

    parser.add_argument('--var_mode', nargs='*', default=['gaussian', 4, 16, True],
    ...
    args.device = torch.device('mps' if torch.cuda.is_available() else 'cpu') # For Apple Silicon Chips 
    # args.device = torch.device('cuda' if torch.cuda.is_available() else 'cpu') # Uncomment this one if running on GPU
    ...
    for n in args.ns:
        ...
        #results_dict = {'elbo': elbo, 'test_lpd': test_lpd, 'asy_log_pDn': asy_log_pDn}
        elbo_dict = {'n': args.sample_size, 
                     'ELBO': elbo.item() + args.nSn.item(), 
                     'base_dist': args.var_mode,
                     'nf_dim': '[{},{}]'.format(args.nf_couplingpair, args.nf_hidden),
                     'seed': args.seed, 'data': args.data}

        path = '{}/{}/{}_{}_{}/sim_size_{}/seed_{}'.format(args.path, args.dataset, args.var_mode[0], args.nf_couplingpair, 
                                     args.nf_hidden, args.sample_size, args.seed)
        ...
        torch.save(elbo_dict, '{}/results.pt'.format(path))
        

        #df = pd.DataFrame(data=elbo_dict)
        #save_to_path = "/Users/seanp/Documents/PythonCode/BNN_via_SLT-main/Results/NF_{}_{}_{}.csv".format(args.base_dist, )
