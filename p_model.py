...
def load_P(model, H, device, prior_mean, prior_var, requires_grad):
    ...
    elif model == 'nonlinreg':
        return NonlinearReg(H, device, prior_mean, prior_var, True, requires_grad)
    elif model == 'nonlinear':
        return Nonlinear(H,device, prior_mean, prior_var, True, requires_grad)
...
class Nonlinear(nn.Module):
    def __init__(self, H, device, prior_mean, prior_var, zeromean, requires_grad):
        ...
        if self.zeromean:
            ...
            self.w3 = torch.nn.Parameter(torch.zeros(self.H, 1), requires_grad=requires_grad)
        else:
            ...
            self.w3 = torch.nn.Parameter(torch.randn(self.H, 1), requires_grad=requires_grad)
    ...
    def load_data(self, sample_size, batch_size):
        ...
        means = self.w1*((self.w2 * X)*(self.w2 * X) + (self.w2 * X)) + (self.w3 * X)
        y_rv = Normal(means, torch.tensor([1.0]))
        Y = y_rv.sample()
    ...
    def loglik(self, x, y, w):
        ...
        for l in range(L):
            means[l, ] = w_a[l]*((w_b[l]*w_b[l] *x.T*x.T) + (w_b[l] * x.T)) + (w_c[l] * x.T)
        means = means.to(self.device)
        y_rv = Normal(means.unsqueeze(dim=2), torch.tensor([1.0]))
        log_p = y_rv.log_prob(y.repeat(1, w.shape[0]).T.unsqueeze(dim=2))

        return log_p

    def loglik_w1_w2(self, x, y, w_a, w_b, w_c):
        L = 1
        B = x.shape[0]
        means = torch.empty(L, B, device=self.device)
        for l in range(L):
            means[l, ] = w_a[l]*((w_b[l] * x.T)*(w_b[l] * x.T) + (w_b[l] * x.T)) + (w_c[l] * x.T)
        means = means.to(self.device)
        y_rv = Normal(means.unsqueeze(dim=2), torch.tensor([1.0]))
...
class NonlinearReg(nn.Module):
    def __init__(self, H, device, prior_mean, prior_var, zeromean, requires_grad):
        ...
        self.w1 = torch.nn.Parameter(torch.zeros(1, 1), requires_grad=requires_grad)
        self.w2 = torch.nn.Parameter(torch.zeros(1, 1), requires_grad=requires_grad)



    def load_data(self, sample_size, batch_size):
        X_dist = Uniform(torch.tensor([-1.0]), torch.tensor([1.0])) # May need to fix this as a sequence 
        X = X_dist.sample(torch.Size([sample_size]))
        # X = torch.linspace(-1, 1, steps = sample_size) 

        #means = torch.matmul(torch.pow(self.w1,4) - torch.matmul(torch.pow(self.w1,2), self.w2) + torch.pow(self.w2,3), X.T)
        means = (torch.pow(self.w1,4) - torch.matmul(torch.pow(self.w1,2), self.w2) + torch.pow(self.w2,3)) * X.T
        y_rv = Normal(means.T, torch.tensor([1.0]))
        Y = y_rv.sample()
        loader = torch.utils.data.DataLoader(TensorDataset(X, Y), batch_size=batch_size, shuffle=True)
        nSn = -y_rv.log_prob(Y).sum()
        return loader, nSn
    ...
    def loglik(self, x, y, w):
        ...
        means = torch.empty(L, B, device=self.device)
        for l in range(L): 
            means[l, ] =  (w_a[l].pow(4) - w_a[l].pow(2) * w_b[l] + w_b[l].pow(3)) * x.T
        means = means.to(self.device)
        y_rv = Normal(means.unsqueeze(dim=2), torch.tensor([1.0]))
        log_p = y_rv.log_prob(y.repeat(1, w.shape[0]).T.unsqueeze(dim=2))
    ...
    def logprior(self, ws):
        #- self.w_dim/2*np.log(2*np.pi) - (1/2)*self.w_dim*np.log(self.prior_var) - torch.diag(torch.matmul(ws-self.prior_mean, (ws-self.prior_mean).T))/(2*self.prior_var)
        # return torch.tensor([0.0]) 
        return - self.w_dim/2*np.log(2*np.pi) \
            - (1/2)*self.w_dim*np.log(self.prior_var) \
            - torch.diag(torch.matmul(ws-self.prior_mean, (ws-self.prior_mean).T))/(2*self.prior_var)
